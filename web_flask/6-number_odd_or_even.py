...
@app.route('/number_odd_or_even/<path>', strict_slashes=False)
def show_your_number_odd_even(path):
    if path.isdigit():
        if int(path) % 2 == 1:
            return render_template('6-number_odd_or_even.html', number=path, oe="odd")
        else:
            return render_template('6-number_odd_or_even.html', number=path, oe="even")
    else:
        abort(404)
